...
/**
 * Atualiza os dados de exibicao do funcionario logado, mantendo o mesmo token.
 * Usado depois que o proprio funcionario altera o nome, para que o cabecalho
 * reflita a mudanca sem precisar entrar de novo.
 */
export function atualizarFuncionarioLocal(funcionario) {
  const sessao = lerSessaoLocal();
  if (!sessao) return;
  gravarSessao({ token: sessao.token, funcionario });
}
/**
 * AUTENTICACAO E ESTADO DO USUARIO (itens 6, 7, 8, 32)
 * ---------------------------------------------------------------------------
